...
>
> **状态：** 功能池、优先级、实现状态与发布规划基线；未签名的 `v0.1.4` preview release 已完成 `DESK-012`、`DESK-023`、`REL-022`、`REL-023`、`REL-024` 的 Windows x64 真机验收与最终 macOS arm64 C0 回归。当前 `v0.1.5` 规划分支以 `REL-028` SEA 单文件 Runtime 为主要范围，同时纳入 `REL-025` 的 macOS 剩余收口、`QA-011` 的 SEA 相关性能基线，以及开发前锁定官方 DSH 发布基线的门禁 `REL-030`；`REL-029` Runtime 依赖深度裁剪明确延期。正式签名分发仍是独立发布门禁。
>
> **日期：** 2026-09-20
...
| REL-030 | 官方 DSH 发布基线门禁 | 在 SEA 开发开始前，查询 DSH 官方 GitHub Releases 和已发布的 `@deepseek-ai/dsh` package metadata，并冻结一个精确的 Compatibility Set 基线。由于 DSH 仍处于 developer preview，当前以 RC 作为相对稳定发布线，因此正式稳定版和官方 RC 都可作为候选；Alpha、Beta、Nightly、分支 HEAD 和无 Tag Commit 仍排除，除非另行明确授权。评审 Release Notes 与已知回归后，优先选择最新的合格官方版本；npm dist-tag 可作为官方推荐依据，但不能替代兼容性验收。如果存在更新的合格版本，必须先原子升级 DSH、生产依赖树、Profile/Bundle 兼容、runtime lock、License/SBOM、迁移保护和回归基线，再开始构建 SEA。禁止浮动版本。2026-09-20 规划检查结果：npm `latest`/`next` 指向 `0.1.5-rc.2`，比当前锁定的 `0.1.5-rc.1` 更新，因此选为 `v0.1.5` 升级候选；`0.1.6-alpha.2` 明确排除。正式实施前须再检查一次，并在开始 SEA 工作前完成 DSH 升级回归 | P0 | 待实现（v0.1.5） |
...
| QA-011 | Performance Regression | 启动、内存、体积和响应趋势 | P2 | 待实现（v0.1.5；v0.1.4 已部分实现）：保留已实现的 v0.1.4 包体积/文件数基线，新增 SEA 启动、内存、安装/运行生成文件数和 Native Cache 回归测量；与本版本无关的联网模型响应基准不纳入本版 |
