...
| REL-024 | 卸载前终止遗留 Sidecar | 卸载程序必须**先检测并终止遗留的 Sidecar 进程**再删除文件，使卸载无需人工介入即可完成。Windows 真机实测：应用进程退出后，其 Sidecar（`runtime\node\win32-x64\node.exe`）成为孤儿进程继续存活，并**内存映射**着若干原生库，导致 NSIS 卸载程序删不掉它们——残留 5 个文件 / 108.5 MB（`node.exe` 93 MB、`sharp` 的 `libvips-42.dll` 18.6 MB 与 `libvips-cpp`/`sharp-win32-x64.node`、`koffi` 的 `koffi.node`），**每一个都恰好是该孤儿进程加载过的库**，系统提示"另一个程序正在使用此文件"。⚠️ **追加实测（提高了严重性）**：孤儿**不止出现在异常路径**——用户**正常关闭应用**后同样残留侧车（观测到父 PID 即刚关闭的应用实例，状态为"已退出"），因此这是**常规使用路径**问题，不是边缘情形。孤儿还会占用仓库 `runtime/`，使 `pnpm check` 报 `os error 32` 失败。待决策范围：用 NSIS 自定义卸载钩子调用既有的 ownership-registry 清理、按安装路径匹配进程的兜底方案，以及**先修正常退出路径**（`RunEvent::ExitRequested` → `stop_runtime()` 是否真正终止了侧车进程树）。验证方式必须是在**存在孤儿 Sidecar 的情况下**真实执行一次卸载 | P1 | 未开始 |
