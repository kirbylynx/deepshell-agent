...
>
> **状态：** 功能池、优先级、实现状态与发布规划基线；未签名的 `v0.1.4` preview release 已完成 `DESK-012`、`DESK-023`、`REL-022`、`REL-023`、`REL-024` 的 Windows x64 真机验收与最终 macOS arm64 C0 回归。当前 `v0.1.5` 版本以 `REL-028` SEA 单文件 Runtime 为主要范围，同时纳入 `REL-025` 的 macOS 剩余收口、`QA-011` 的 SEA 相关性能基线，以及开发前锁定官方 DSH 发布基线的门禁 `REL-030`；`REL-029` Runtime 依赖深度裁剪明确延期。SEA 工作已完成 Windows x64 W0–W3 在最终 clean rebuild 产物上的验收（含真机工具复测）与 macOS arm64 轮次；combined staging、macOS 基于最终 `v0.1.5` 提交的重建与发布仍待完成。正式签名分发仍是独立发布门禁。
>
> **日期：** 2026-09-26
...
- [`docs/releases/v0.1.4.zh.md`](releases/v0.1.4.zh.md)
...
- `已实现（v0.1.4）` 表示该功能已在 `v0.1.4` 打包效率发布候选中落地，适用的 Windows x64 与 macOS arm64 验收事实已登记在 `docs/releases/v0.1.4.zh.md`；它是实现事实，不代表 tag 或二进制已经发布。
- `待实现（v0.1.5）` 表示该功能已进入当前 `v0.1.5` SEA Runtime 规划/开发分支，但尚未作为已实现功能落地。
...
| DESK-012 | 基础菜单 | About、Quit 和版本信息 | P0 | 已实现（v0.1.4） |
...
| REL-028 | SEA 单文件 Runtime | 将安装目录中的完整 Node.js + DSH 依赖树替换为平台专属的 Single Executable Application（SEA，单一可执行应用）侧车，使基础应用不再安装或解压数万个运行时文件。这是解决安装、解压、卸载、杀毒扫描、索引和文件元数据开销的主要必做方向。生产路线只采用一套锁定版本的 `@yao-pkg/pkg` Enhanced SEA；原生 Node SEA 仅在兼容性 POC 失败时作为重新决策的备选，不并行实现，也不作为发布产物。Release `.app`/DMG/NSIS/portable ZIP 只能携带 SEA Runtime，不得内置旧 Node + `node_modules` 作为备用 Runtime；开发环境可以保留标准 Runtime 作为显式对照路径，但产物校验必须证明它未进入 Release。Runtime 失败必须 fail closed 并保留诊断：发布前发现阻断问题则停止发布，发布后发现严重问题则撤回对应版本并回退至上一产品版本，不得在应用内部静默切换 Runtime 实现。可变的 `DSH_HOME`、用户安装扩展、凭据、Session、日志和 Native Addon 缓存仍保留在只读嵌入文件系统之外。验收须覆盖 macOS arm64 和 Windows x64、官方 DSH Web UI 启动、Coding/Work Mode、Provider 与 Session 流程、Web Search/Fetch、外部 Plugin 解析、Worker/Native Addon、生命周期清理、签名、产物清单、回滚，以及真实安装与运行生成文件数。不得改变既有 Tauri/DSH 架构边界，不得依赖 DSH 私有 UI 接口 | P1 | 已实现（v0.1.5）：Windows x64 SEA 已在真机验收（W0–W3，clean rebuild 产物、on-device 证据、用户真机工具复测）；macOS arm64 SEA 已在 macOS 轮次验收（MAC-01…MAC-09）；两平台在 combined release staging 时基于最终 `v0.1.5` 提交重建 |
| REL-029 | Runtime 依赖深度裁剪 | 在 SEA 基线已经可运行且完成测量后，可选地通过可重复、按目标平台、package-aware（包感知）的裁剪减少构建输入和最终可执行文件体积。高置信度裁剪可移除开发 headers、非目标平台 Native Binary、测试/示例及其他已验证与运行无关的文件；只有实测体积收益足以覆盖升级和回归成本时，才采用更深入的逐包裁剪。规则必须作用于生成的 staging 临时树，不得修改锁定的原始 Runtime；必须保留所需 License/Notice 和动态资源，输出裁剪前后清单，遇到未知依赖布局时失败关闭，并重新执行完整 DSH 升级、Runtime 与打包回归。本项不得阻塞或成为 REL-028 的前置条件，并明确不纳入 `v0.1.5` | P2 | 未开始 |
| REL-030 | 官方 DSH 发布基线门禁 | 在 SEA 开发开始前，查询 DSH 官方 GitHub Releases 和已发布的 `@deepseek-ai/dsh` package metadata，并冻结一个精确的 Compatibility Set 基线。由于 DSH 仍处于 developer preview，当前以 RC 作为相对稳定发布线，因此正式稳定版和官方 RC 都可作为候选；Alpha、Beta、Nightly、分支 HEAD 和无 Tag Commit 仍排除，除非另行明确授权。评审 Release Notes 与已知回归后，优先选择最新的合格官方版本；npm dist-tag 可作为官方推荐依据，但不能替代兼容性验收。如果存在更新的合格版本，必须先原子升级 DSH、生产依赖树、Profile/Bundle 兼容、runtime lock、License/SBOM、迁移保护和回归基线，再开始构建 SEA。禁止浮动版本。2026-09-20 规划检查结果：npm `latest`/`next` 指向 `0.1.5-rc.2`，比当前锁定的 `0.1.5-rc.1` 更新，因此选为 `v0.1.5` 升级候选；`0.1.6-alpha.2` 明确排除。正式实施前须再检查一次，并在开始 SEA 工作前完成 DSH 升级回归 | P0 | 已实现（v0.1.5）：已完成再检查并冻结于 `@deepseek-ai/dsh@0.1.5-rc.2`（npm `latest`/`next`；官方 tag `dsh-v0.1.5-rc.2`；`0.1.6-alpha.2` 排除）；锁定集合未发生浮动（见 [`docs/plans/v0.1.5-sea-runtime/p0-baseline-freeze.md`](plans/v0.1.5-sea-runtime/p0-baseline-freeze.md)） |
...
| QA-011 | Performance Regression | 启动、内存、体积和响应趋势 | P2 | 已实现（v0.1.5）：保留已实现的 v0.1.4 包体积/文件数基线，新增 SEA 启动、内存、安装/运行生成文件数和 Native Cache 回归测量（benchmark、on-device 证据、size gate、package report；两项用户已确认的门槛例外已声明并披露）；与本版本无关的联网模型响应基准不纳入本版 |
