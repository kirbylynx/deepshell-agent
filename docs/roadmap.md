...
>
> **Status:** Feature pool, priority, implementation status, and release-planning baseline; the unsigned `v0.1.4` preview release completed Windows x64 on-device acceptance and final macOS arm64 C0 regression for `DESK-012`, `DESK-023`, `REL-022`, `REL-023`, and `REL-024`. The `v0.1.5` release selects `REL-028` SEA single-file Runtime as its primary scope, together with the remaining macOS closure of `REL-025`, the SEA-related performance baseline of `QA-011`, and the pre-development official DSH release-baseline gate `REL-030`; `REL-029` deep runtime dependency pruning is explicitly deferred. The SEA work completed Windows x64 W0–W3 acceptance on the final clean-rebuild artifacts (including the real-device tool re-test) and the macOS arm64 round; combined staging, the macOS rebuild from the final `v0.1.5` commit, and publication remain pending. Formal signed distribution remains a separate release gate.
>
> **Date:** 2026-09-26
...
- `Implemented (v0.1.3)` means the feature has landed in the `v0.1.3` Windows x64 platform-acceptance baseline and is covered by the current validation scope.
...
- `Planned (v0.1.4)` means the feature is selected for the active `v0.1.4` packaging-efficiency branch, but has not yet landed as an implemented feature.
- `Implemented (v0.1.4)` means the feature has landed in the `v0.1.4` packaging-efficiency release candidate and its applicable Windows x64 and macOS arm64 acceptance is recorded in `docs/releases/v0.1.4.md`. It is an implementation fact, not a claim that tag or binary publication has already occurred.
- `Planned (v0.1.5)` means the feature is selected for the active `v0.1.5` SEA-runtime planning/development branch, but has not yet landed as an implemented feature.
...
| DESK-007 | Sidecar health check | Distinguish starting, ready, and crashed (the original description included degraded, but `v0.1.3` code reading confirms RuntimePhase has only Starting/Ready/Recovering/StartupFailed/RuntimeFailed - **degraded does not exist in the implementation**) | P0 | Implemented (v0.1.0) |
...
| DESK-012 | Basic menu | About, Quit, and version information | P0 | Implemented (v0.1.4) |
...
| SESSION-022 | Real old-session upgrade acceptance | Manually validate that representative `v0.1.1` Sessions open or migrate correctly under `v0.1.2` before promotion beyond developer preview | P0 | Not implemented: skipped by user decision (no old-version data available) |
...
| SAFE-012 | Platform Sandbox capability matrix | Show actual macOS/Windows/Linux protection and degradation | P1 | Partially implemented (v0.1.3): of 8 Windows items, 5 measured and passed, 1 identified and recorded, 2 unverified; macOS not re-measured, Linux not accepted |
...
| OPS-012 | Uninstall data-retention policy | Retain by default and provide an explicit cleanup method | P2 | Partially implemented (v0.1.3): "retained by default" measured and passed; the actual deletion list for the delete checkbox was not measured; incomplete uninstall cleanup is a known defect |
...
| REL-014 | Cross-WebView acceptance | WKWebView, WebView2, and WebKitGTK difference testing | P1 | Partially implemented (v0.1.3): of 6 Windows/WebView2 dimensions, 1 passed, 3 partially measured, 2 not measured; WKWebView not re-measured, WebKitGTK not accepted |
...
| REL-017 | Windows x64 installer validation | Validate the actual NSIS installer, WebView2 first run, Session creation, shutdown cleanup, and uninstall behavior on real Windows or CI | P1 | Implemented (v0.1.3): end-to-end acceptance on real Windows x64 hardware; the uninstall step carries a known defect (see REL-024) |
...
| REL-020 | Windows x64 build-environment acceptance | Measure and record build-host prerequisites on real Windows x64: MSVC C++ toolset and Windows SDK completeness, `rc.exe` availability, WebView2 Runtime, PowerShell resolution path, how the NSIS toolchain is obtained, and the `pnpm release:windows:check` preflight result | P1 | Implemented (v0.1.3): all six prerequisites measured and recorded |
| REL-021 | Windows x64 package-manifest and security-boundary acceptance capability | Enable the Windows side to produce an equivalently structured package manifest and run security-boundary comparison, covering test-resource and E2E-marker checks; the change must preserve the existing macOS-side capability | P1 | Implemented (v0.1.3): manifest emission and security-boundary comparison both executed on Windows |
| REL-022 | Per-platform runtime bundling | Each platform installer must bundle only the Node runtime it actually needs, instead of packaging every platform's runtime into every installer. Measured on the `v0.1.3` Windows NSIS installer: 32226 files / 512 MB installed, of which `runtime/node/darwin-arm64` contributes 4800 files / 187.5 MB that Windows never uses; the macOS `.app` carries the symmetric `win32-x64` waste. Fixing it requires per-platform conditional `bundle.resources` mapping in `tauri.conf.json` **and** updating the `verify-package.mjs` required-artifact assertion that currently demands both platform runtimes inside one artifact. Must not be achieved by removing official DSH core runtime capability | P1 | Implemented (v0.1.4) |
| REL-023 | Windows x64 portable (no-install) ZIP | Ship a second Windows x64 artifact alongside the NSIS installer: a portable ZIP that runs without installation (extract and launch). Confirmed `v0.1.4` scope: portable uses the same `%APPDATA%\com.deepshell.agent` data directory as the installed app, keeps the current `credentials-local` and Permission/Approval semantics, creates no DeepShell uninstall registry entry, desktop shortcut, or Start Menu shortcut, supports same-version installed/ZIP coexistence on disk and alternating use through the existing single-instance lock, does not support concurrent independent instances, and relies on the system Evergreen WebView2 Runtime instead of bundling a bootstrapper/offline/fixed runtime in the ZIP. Must reuse the existing multi-artifact release-staging and package-manifest checks rather than inventing a new manifest | P1 | Implemented (v0.1.4) |
| REL-024 | Terminate leftover Sidecar before uninstalling | The uninstaller must detect and terminate any leftover Sidecar process before removing files, so uninstall completes without manual intervention. Observed on real Windows: after the app process exited, its Sidecar (`runtime\node\win32-x64\node.exe`) survived as an orphan and kept native libraries memory-mapped, so the NSIS uninstaller could not delete them — 5 files / 108.5 MB remained (`node.exe` 93 MB, `sharp`'s `libvips-42.dll` 18.6 MB and `libvips-cpp`/`sharp-win32-x64.node`, `koffi`'s `koffi.node`), each one exactly a library the orphan had loaded. ⚠️ **Additional measurement that raises severity**: orphans do **not** require an abnormal path — after the user **closed the app normally**, a leftover Sidecar was still observed (its parent PID was the just-closed app instance, already exited), so this is a **regular-use** problem rather than an edge case. Orphans also hold the repository `runtime/`, making `pnpm check` fail with `os error 32`. Confirmed `v0.1.4` scope: fix the normal-exit cleanup path first, then add an NSIS `PREUNINSTALL` hook that calls the existing ownership-registry cleanup; if no ownership record exists, fallback may only inspect the build-time allowlist of absolute normalized executable paths under the current normalized install root, never terminate by process name, and must fail closed when ownership or path identity is uncertain. Verification must be a real uninstall performed **while an orphan Sidecar exists**, plus forged-record, missing-record, other-directory, and timeout fault injection | P1 | Implemented (v0.1.4) |
| REL-025 | Quarantined ownership record must not block startup permanently | `recover_registered` must be able to retire a stale quarantined ownership record instead of failing every subsequent launch. Measured on real Windows (`v0.1.3`): a `runtime-state/ownership.json` left in `state:"quarantined"` made the app fail on **every** startup with `runtime_stop_failed` and no recoverable path — the state is only ever *written* (by `quarantine_registered` and by the executable-mismatch branch) and never *retired*, because `ensure_active` rejects `quarantined` unconditionally while `recover_registered` only re-verifies records whose `expected_executable` matches. The documented promise ("re-verify and clean up on next startup") was never implemented. The Windows side was fixed in `v0.1.3` for the case where the recorded process is already dead; **the macOS `process_tree.rs` carries the same defect and was deliberately left unchanged** because it cannot be verified on Windows hardware. Remaining scope: port the fix to macOS with real macOS verification. The `v0.1.5` decision is to preserve fail-closed behavior for a recorded process that is alive but foreign or whose identity cannot be proven; the app must not delete its record or terminate that process automatically | P1 | Planned (v0.1.5): Windows fix landed in v0.1.3; macOS closure selected for v0.1.5 |
| REL-026 | Fix file drag-and-drop being silently swallowed by the Tauri layer | The official DSH Web UI implements HTML5 file drag-and-drop and renders a drop affordance, yet **dropping a file into DeepShell does nothing**. Measured on real Windows (`v0.1.3`): the user reported that the window "shows it can be dropped, but nothing happens when you drop it". Root cause located: the `tauri.conf.json` window config does not set `dragDropEnabled`, and Tauri 2 defaults that option to **`true`**, whose semantics are to **block native HTML5 drag-and-drop** and instead emit a Tauri `DragDrop` event to the app; meanwhile `src-tauri/src/**.rs` contains **no handling whatsoever** for `DragDrop`/`on_drop`/`DragDropEvent` (a full search returns zero hits), so nothing receives the event and the drop is silently discarded. **Not Windows-specific** — `dragDropEnabled` is a cross-platform setting, so macOS should carry the same defect. Fix directions (must be verified by a real WebView2 run, not inferred from configuration): (1) set `dragDropEnabled: false` so the WebView handles HTML5 drops itself (smallest change); or (2) keep `true` and implement `DragDrop` event forwarding in Rust (larger change that must interoperate with the official UI`s existing event contract, risking the "no private frontend interfaces" constraint) | P1 | Not started |
| REL-027 | Detect and recover from a dead client renderer process | The app monitors **Sidecar** health completely (process liveness, `ownership.json`, crash detection, auto-restart, a 60-second restart budget) but monitors **its own client renderer** not at all. Measured on real Windows (`v0.1.3`): after `taskkill` removed the WebView2 process tree, the app process stayed alive, the window handle remained, and the Sidecar stayed fully healthy (port normal, registry `active`) - yet the client **was never recreated** (process count stayed 0 for 60 s and still 0 at 90 s) and the app log gained **no new events at all**, neither reporting a client fault nor attempting recovery. What the user saw was "the window is still there, but the content is pure black, nothing at all", with **no notification whatsoever**; the only way out is to force-kill the process and relaunch. Code reading confirms it: a full search for `RenderProcess`/`ProcessFailed`/`reload`/`Recreate` returns **zero hits**, and the only crash path is the Sidecar's `runtime_crashed`. This is a **realistically reachable** scenario (renderer processes die on Windows from GPU crashes, memory pressure, and driver faults) and is **not Windows-specific** (Tauri is cross-platform; a WKWebView content process can be killed the same way). Architecture reference: section 10.2 lists `Desktop ready` as one of six fault domains and forbids collapsing them into a single online/offline state; this scenario is "**Desktop not ready, and nobody knows**". Scope: detect renderer exit (via Windows process/window events or a webview-side heartbeat), reload or recreate the webview on failure, and report it to the user honestly | P1 | Not started |
| REL-028 | SEA single-file Runtime | Replace the installed full Node.js + DSH dependency tree with a platform-specific Single Executable Application (SEA) sidecar so the base app no longer installs or extracts tens of thousands of runtime files. This is the primary, required direction for solving install/extract/uninstall, antivirus-scan, indexing, and metadata overhead. Use one pinned `@yao-pkg/pkg` Enhanced SEA production route; raw Node SEA is only a decision fallback if the compatibility POC fails, not a parallel implementation or release artifact. Release `.app`/DMG/NSIS/portable ZIP artifacts must contain only the SEA Runtime and must not carry the old Node + `node_modules` tree as an internal fallback; development may retain the standard Runtime as an explicit comparison path, but package verification must prove it is absent from Release artifacts. Runtime failure is fail-closed with diagnostics: block the release before publication, or withdraw the affected release and roll back to the preceding product version after publication, rather than silently switching Runtime implementations. Retain mutable `DSH_HOME`, user-installed extensions, credentials, Sessions, logs, and native-addon cache outside the read-only embedded filesystem. Acceptance must cover macOS arm64 and Windows x64, official DSH Web UI startup, Coding/Work modes, Provider and Session flows, Web Search/Fetch, external Plugin resolution, Worker/native addons, lifecycle cleanup, signing, package manifests, rollback, and actual installed/generated file counts. This work must preserve the existing Tauri/DSH architecture boundary and must not depend on private DSH UI interfaces | P1 | Implemented (v0.1.5): Windows x64 SEA landed and verified on real hardware (W0–W3 on clean-rebuild artifacts, on-device evidence, user tool re-test); macOS arm64 SEA landed and verified in the macOS round (MAC-01…MAC-09); both platforms are rebuilt from the final `v0.1.5` commit during combined release staging |
| REL-029 | Deep runtime dependency pruning | After the SEA baseline is functional and measured, optionally reduce its build input and final binary size through reproducible, target-specific, package-aware pruning. High-confidence pruning may remove development headers, non-target native binaries, tests/examples and other verified runtime-irrelevant files; deeper per-package pruning is adopted only when measured size benefit justifies its upgrade and regression cost. Rules must run against a generated staging tree rather than mutate the pinned source runtime, preserve required licenses/notices and dynamic assets, emit before/after manifests, fail closed on an unknown dependency layout, and rerun the full DSH upgrade/runtime/package regression suite. This optimization must not block or become a prerequisite for REL-028 and is explicitly out of scope for `v0.1.5` | P2 | Not started |
| REL-030 | Official DSH release baseline gate | Immediately before SEA implementation begins, query the official DSH GitHub releases and the published `@deepseek-ai/dsh` package metadata, then freeze one exact compatibility-set baseline. Because DSH is still in developer preview and currently publishes RCs as its relatively stable release line, both a formal stable release and an official RC are eligible; alpha, beta, nightly, branch heads, and untagged commits remain excluded unless separately authorized. Prefer the newest eligible official release after reviewing its release notes and known regressions; an npm dist-tag is evidence of the official recommendation but does not replace compatibility acceptance. If a newer eligible release exists, atomically upgrade DSH, its production dependency tree, profile/bundle compatibility, runtime lock, licenses/SBOM, migration guards, and regression baseline before building SEA. Never float versions. Planning-time check on 2026-09-20: npm `latest`/`next` points to `0.1.5-rc.2`, which is newer than the pinned `0.1.5-rc.1` and is therefore the selected upgrade candidate for `v0.1.5`; `0.1.6-alpha.2` is excluded. Recheck immediately before implementation, then complete the DSH upgrade regression before SEA work begins | P0 | Implemented (v0.1.5): recheck performed and frozen at `@deepseek-ai/dsh@0.1.5-rc.2` (npm `latest`/`next`; official tag `dsh-v0.1.5-rc.2`; `0.1.6-alpha.2` excluded); the pinned compatibility set did not float (see [`docs/plans/v0.1.5-sea-runtime/p0-baseline-freeze.md`](plans/v0.1.5-sea-runtime/p0-baseline-freeze.md)) |
...
| QA-011 | Performance Regression | Startup, memory, package size, and response trends | P2 | Implemented (v0.1.5): retains the implemented v0.1.4 package-size/file-count baseline and adds SEA startup, memory, installed/generated-file, and native-cache regression measurements (benchmark, on-device evidence, size gates, package report; the two user-confirmed gate exceptions are declared and disclosed); unrelated network-model response benchmarking remains outside this version |
