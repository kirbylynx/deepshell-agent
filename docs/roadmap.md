...
>
> **Status:** Feature pool, priority, implementation status, and release-planning baseline; `v0.0.1` POC through `v0.1.3` Windows x64 platform acceptance are closed, and `DESK-023`, `REL-022`, `REL-023`, `REL-024`, and `QA-011` have landed on the active `v0.1.4` packaging-efficiency branch with Windows x64 on-device acceptance recorded (macOS final regression C0 still pending).
>
> **Date:** 2026-09-15
...
- `Implemented (v0.1.4, Windows)` means the feature has landed on the active `v0.1.4` packaging-efficiency branch and the Windows x64 on-device acceptance is recorded in `docs/releases/v0.1.4.md`; the macOS final regression (C0) and release closeout are still pending, so this status must not be read as a completed cross-platform release.
...
| QA-011 | Performance Regression | Startup, memory, package size, and response trends | P2 | Implemented (v0.1.4, Windows): package-size and file-count regression coverage; macOS measurements follow the C0 rebuild |
