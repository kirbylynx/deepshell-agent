...
>
> **Status:** Feature pool, priority, implementation status, and release-planning baseline; `v0.0.1` POC through `v0.1.3` Windows x64 platform acceptance are closed, `DESK-023`, `REL-022`, `REL-023`, and `REL-024` have landed on the active `v0.1.4` packaging-efficiency branch with Windows x64 partial on-device acceptance recorded, and the `QA-011` package-size/file-count slice is partially implemented (macOS final regression C0 still pending).
>
> **Date:** 2026-09-15
...
- `Implemented (v0.1.4, Windows partial acceptance)` means the feature has landed on the active `v0.1.4` packaging-efficiency branch and the Windows x64 partial on-device acceptance is recorded in `docs/releases/v0.1.4.md`; the macOS final regression (C0) and release closeout are still pending, so this status must not be read as a completed cross-platform release.
...
| DESK-012 | Basic menu | About, Quit, and version information | P0 | Partially implemented (HIG-compliant on macOS; Windows completion is planned for `v0.1.4`, see DESK-023) |
...
| QA-011 | Performance Regression | Startup, memory, package size, and response trends | P2 | Partially implemented (v0.1.4): package size and file-count coverage only; startup, memory, and response trends are not started |
