...
| REL-022 | Per-platform runtime bundling | Each platform installer must bundle only the Node runtime it actually needs, instead of packaging every platform's runtime into every installer. Measured on the `v0.1.3` Windows NSIS installer: 32226 files / 512 MB installed, of which `runtime/node/darwin-arm64` contributes 4800 files / 187.5 MB that Windows never uses; the macOS `.app` carries the symmetric `win32-x64` waste. Fixing it requires per-platform conditional `bundle.resources` mapping in `tauri.conf.json` **and** updating the `verify-package.mjs` required-artifact assertion that currently demands both platform runtimes inside one artifact. Must not be achieved by removing official DSH core runtime capability | P1 | Not started |
| REL-023 | Windows x64 portable (no-install) ZIP | Ship a second Windows x64 artifact alongside the NSIS installer: a portable ZIP that runs without installation (extract and launch). Intended for evaluation, locked-down machines, and users who cannot run an installer. Requires deciding and documenting: (1) where a portable run stores its data — the shared installer `%APPDATA%\com.deepshell.agent` or a relocated per-portable-directory path (`paths.rs` currently derives everything from Tauri's `app_data_dir()`); (2) whether uninstall/cleanup guidance accompanies it, since no uninstaller exists; (3) how to avoid registry writes (currently the NSIS uninstall entry is written by the installer only) and shortcut creation; (4) whether it must coexist with an installed copy without Session/credential conflicts. Must reuse the existing NSIS multi-artifact release-staging and package-manifest checks rather than inventing a new manifest | P1 | Not started |
| REL-024 | Terminate leftover Sidecar before uninstalling | The uninstaller must detect and terminate any leftover Sidecar process before removing files, so uninstall completes without manual intervention. Observed on real Windows: after the app process exited, its Sidecar (`runtime\node\win32-x64\node.exe`) survived as an orphan and kept native libraries memory-mapped, so the NSIS uninstaller could not delete them — 5 files / 108.5 MB remained (`node.exe` 93 MB, `sharp`'s `libvips-42.dll` 18.6 MB and `libvips-cpp`/`sharp-win32-x64.node`, `koffi`'s `koffi.node`), each one exactly a library the orphan had loaded. ⚠️ **Additional measurement that raises severity**: orphans do **not** require an abnormal path — after the user **closed the app normally**, a leftover Sidecar was still observed (its parent PID was the just-closed app instance, already exited), so this is a **regular-use** problem rather than an edge case. Orphans also hold the repository `runtime/`, making `pnpm check` fail with `os error 32`. Scope to decide: an NSIS custom uninstall hook that runs the existing ownership-registry cleanup, a build-time-safe fallback that matches processes by install path, and **fixing the normal exit path first** (whether `RunEvent::ExitRequested` → `stop_runtime()` actually terminates the Sidecar process tree). Verification must be a real uninstall performed **while an orphan Sidecar exists** | P1 | Not started |
