...
describe('DSH 上游边界', () => {
  ...
  it('显式拒绝新 WebView，并限制 Tauri Capability 为本地 Bootstrap', async () => {
    ...
    const windowsProcessTree = await readFile(resolve(root, 'src-tauri/src/sidecar/process_tree_windows.rs'), 'utf8')
    ...
    expect(webview).toContain('Command::new("rundll32.exe")')
    expect(webview).toContain('url.dll,FileProtocolHandler')
    expect(webview).not.toContain('Command::new("cmd")')
    expect(windowsProcessTree).toContain('Command::new("explorer.exe")')
    expect(windowsProcessTree).not.toContain('args(["/C", "start"')
