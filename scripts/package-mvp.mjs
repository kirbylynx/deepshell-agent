...
function run(command, args) {
  return new Promise((resolveRun, reject) => {
    const plan = packageCommandPlan(command, args)
...
if (process.platform === 'darwin') {
  await run('node', ['scripts/build-package.mjs', 'release'])
  // Tauri 的 beforeBuildCommand 会重新准备 Runtime/Profile；构建完成后必须复验最终输入。
  await run(pnpm, ['runtime:verify'])
  await run(pnpm, ['profile:verify'])
  ...
  await run('node', ['scripts/capture-package-manifest.mjs', 'release', 'macos-app'])
  await run('node', ['scripts/capture-package-manifest.mjs', 'release', 'macos-dmg'])
